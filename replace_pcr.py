...
def npareatotal(values, areaclass):
    ...
    """
    shp_orig = values.shape
    areaclass_ = areaclass.flatten().astype(np.int64)
    areaclass_[areaclass_ < 0] = 256
    grpd = np.take(np.bincount(areaclass_, weights=values.flatten()), areaclass_)
    return grpd.reshape(shp_orig)
